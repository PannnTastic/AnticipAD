#!/usr/bin/env julia
# SOLVER: SARSOP (3 states, max_steps=20, N=100k)
...
function main()
    println("=" ^ 70); println("SOLVER: SARSOP (3 states | max_steps=$MAX_STEPS)"); println("=" ^ 70)
    println("SARSOP training typically takes 30-90 seconds for 3 states.")
    println("Started at: ", now())
    pomdp = AlzheimerPOMDPProblem(discount=0.95)
    
    println("\nTraining SARSOP (precision=1e-2, timeout=300s)...")
    t_train = time()
    solver = SARSOPSolver(precision=1e-2, timeout=300.0)
    policy = solve(solver, pomdp)
    train_time = round(time()-t_train, digits=2)
    println("Training completed in $(train_time)s")
    ...
    println("\nEvaluating on $N_SAMPLES patients (max_steps=$MAX_STEPS)...")
    t0 = time()
    results = [run_episode(pomdp, policy, updater, b0, s; max_steps=MAX_STEPS, rng=rng) for s in true_states]
    eval_time = round(time()-t0, digits=2)
    println("Evaluation completed in $(eval_time)s")
    
    save_results("results_sarsop_$(N_SAMPLES).json", results, train_time, eval_time)
    print_summary(results)
end

function save_results(filename, data, train_time, eval_time)
    out = Dict("solver"=>"SARSOP", "n_samples"=>N_SAMPLES, "max_steps"=>MAX_STEPS, "seed"=>SEED,
        "train_time_sec"=>train_time, "eval_time_sec"=>eval_time,
