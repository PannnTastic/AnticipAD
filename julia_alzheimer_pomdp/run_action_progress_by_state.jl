#!/usr/bin/env julia
# Generate Action Progress Figures by True State
# 3 figures: CN patients, MCI patients, Dementia patients
# Each figure shows 4 solvers (SARSOP, MyopicPOMDP, Expert, Random)
...
function main()
    ...
    println("N=$N_SAMPLES patients per state | Max Steps=$MAX_STEPS")
    println("=" ^ 70)
    ...
    for true_clinical in [:CN, :MCI, :Dementia]
        println("\n--- Processing true state: $true_clinical ---")
        ...
        println("  Generating figure for $true_clinical...")
...
function generate_state_figure(all_data, true_state_name::Symbol)
    ...
    plot!(plot_title="Action Distribution — True State: $state_title | Correct: $correct_action | N=$N_SAMPLES per solver",
          plot_titlefontsize=11)

    mkpath("figures_paper")
    filename_base = "v2_07_action_progress_$(lowercase(string(true_state_name)))"
