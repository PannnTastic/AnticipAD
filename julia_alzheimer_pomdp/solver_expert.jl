#!/usr/bin/env julia
# SOLVER: Expert Policy (3 states, max_steps=20, N=100k)
...
function main()
    println("=" ^ 70); println("SOLVER: Expert Policy (3 states | max_steps=$MAX_STEPS)"); println("=" ^ 70)
    ...
    save_results("results_expert_$(N_SAMPLES).json", results)
    print_summary(results)
end

function save_results(filename, data)
    out = Dict("solver"=>"Expert", "n_samples"=>N_SAMPLES, "max_steps"=>MAX_STEPS, "seed"=>SEED,
